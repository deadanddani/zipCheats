  ...
  solve(resolution, timeBudgetMs = 10000) {
    ...
    while (!resolution.isCompleted()) {
      ...
      if (performance.now() > deadline) {
        console.warn(`[zipCheats] Solver (${this.method.name}) timed out after ${timeBudgetMs} ms`);
        return false;
      }
    ...
    console.log(
      `[zipCheats] Solver (${this.method.name}) finished in ${(performance.now() - startTime).toFixed(2)} ms with ${iterations} iterations.`,
    );
    return solved;
  }
}

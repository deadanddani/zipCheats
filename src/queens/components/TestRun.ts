...
import { renderQueensBoard } from './QueensBoard'
...
// (5x5, 9x9, ...) never changes how much space the grid takes up on screen
...
function fitBoardToFrame(board: HTMLElement, map: QueensMap) {
  ...
  function showRun(maps: SavedMapSummary[]) {
    ...
    async function run() {
      ...
      for (let i = 0; i < maps.length; i++) {
        ...
        const board = renderQueensBoard(map, resolution, { interactive: false })

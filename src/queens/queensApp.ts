...
import { renderQueensBoard } from './components/QueensBoard'
...
/**
 * Mounts the Queens game into `app`. Mirrors the Zip app: a home screen with a
 * map browser, and a board view with manual play + a (stub) solve button.
 */
export function mountQueens(app: HTMLDivElement, onChangeGame: () => void) {
  ...
  function showGrid(map: QueensMap) {
    linkedinRow.remove()
    browser.remove()
    testRunBtn.remove()
    ...
    boardEl = renderQueensBoard(map, currentResolution)
